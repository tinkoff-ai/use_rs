...
class SLRCPlus2(SequentialModel):
    reader = 'KGReader'
    ...
    def _define_params(self):
        self.u_embeddings = nn.Embedding(self.user_num, self.emb_size, padding_idx=0)
    ...
    def forward(self, feed_dict):
        ...
        history_item = feed_dict['history_items']
        # Excitation
        ...
        user_seq_emb = self.u_embeddings(history_item)
        user_seq_emb_dropout = self.emb_dropout(user_seq_emb)
        gru_output, u_vectors = self.gru_layers(user_seq_emb_dropout)
        cf_u_vectors = u_vectors[1]
        cf_u_vectors = self.dense(cf_u_vectors)
